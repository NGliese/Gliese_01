...
Socket Socket::accept() {
	...
	socklen_t sin_size;
	int clientSockFD = ::lwip_accept_r(m_sock,  (struct sockaddr *)&client_addr, &sin_size); // @suppress("Function cannot be resolved")
...
std::string Socket::addressToString(struct sockaddr* addr) {
	...
	sprintf(temp, "%s [%d]", ip, ntohs(pInAddr->sin_port)); // @suppress("Invalid arguments")
...
int Socket::bind(uint16_t port, uint32_t address) {
	...
	serverAddress.sin_addr.s_addr = htonl(address); // @suppress("Invalid arguments")
	serverAddress.sin_port        = htons(port); // @suppress("Invalid arguments")
	int rc = ::lwip_bind_r(m_sock, (struct sockaddr *)&serverAddress, sizeof(serverAddress)); // @suppress("Function cannot be resolved")
	if (rc != 0) {
...
int Socket::close() {
	...
	if (m_sock != -1) {
		...
		rc = ::lwip_close_r(m_sock); // @suppress("Function cannot be resolved")
		if (rc != 0) {
...
int Socket::connect(struct in_addr address, uint16_t port) {
	...
	createSocket();
	int rc = ::lwip_connect_r(m_sock, (struct sockaddr *)&serverAddress, sizeof(struct sockaddr_in)); // @suppress("Function cannot be resolved")
	if (rc == -1) {
...
void Socket::getBind(struct sockaddr* pAddr) {
	...
	int rc = ::getsockname(m_sock, pAddr, &nameLen); // @suppress("Invalid arguments")
	if (rc != 0) {
...
int Socket::listen(uint16_t port, bool isDatagram, bool reuseAddress) {
	...
	// sockets.
	if (!isDatagram) {
		rc = ::lwip_listen_r(m_sock, 5); // @suppress("Function cannot be resolved")
		if (rc == -1) {
...
{
    int res = ::setsockopt(m_sock, SOL_SOCKET, option, value, len); // @suppress("Invalid arguments")
    if(res < 0) {
...
size_t Socket::receive(uint8_t* data, size_t length, bool exact) {
	...
	if (exact == false) {
		...
		if (getSSL()) {
			...
			rc = ::lwip_recv_r(m_sock, data, length, 0); // @suppress("Function cannot be resolved")
			if (rc == -1) {
	...
	while(amountToRead > 0) {
		if (getSSL()) {
			...
			rc = ::lwip_recv_r(m_sock, data, amountToRead, 0); // @suppress("Function cannot be resolved")
		}
		if (rc == -1) {
			ESP_LOGE(LOG_TAG, "receive: %s", strerror(errno));
			return 0;
...
int Socket::receiveFrom(uint8_t* data, size_t length,	struct sockaddr *pAddr) {
	...
	int rc = ::recvfrom(m_sock, data, length, 0, pAddr, &addrLen); // @suppress("Invalid arguments")
	return rc;
} // receiveFrom
...
int Socket::send(const uint8_t* data, size_t length) const {
    ...
    while (length > 0)
    {
        if (getSSL()) {
            ...
            rc = ::lwip_send_r(m_sock, data, length, 0); // @suppress("Function cannot be resolved")
...
void Socket::sendTo(const uint8_t* data, size_t length, struct sockaddr* pAddr) {
	...
	if (getSSL()) {
		...
		rc = ::sendto(m_sock, data, length, 0, pAddr, sizeof(struct sockaddr)); // @suppress("Invalid arguments")
	}
	if (rc < 0) {
